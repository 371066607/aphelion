...
test('expedition state: 三类 objective 只接受 CFG 中真实且类别匹配的物品',()=>{
  ...
  ['resources','samples','relics'].forEach(kind=>{
    ...
    const out=E.validateSelection(s.meta,['r1'],{food:1},kind);
  ...
  try{
    ...
    const bad=E.validateSelection(s.meta,['r1'],{food:1},'samples');
...
test('expedition state: 显式零补给和省略补给均可出发且不扣库存',()=>{
  let s=fixture(),out=E.begin(s.meta,s.colony,['r1'],{food:0},'resources');
  if(!out.ok||out.run.supply.food!==0||s.meta.res.food!==8)throw new Error('food=0 应可出发');
  s=fixture();out=E.begin(s.meta,s.colony,['r1'],{},'resources');
...
test('expedition state: begin 生成稳定 runId 并只改变所选居民 worldId',()=>{
  ...
  const out=E.begin(s.meta,s.colony,['r1','r3'],{food:3},'samples');
  ...
  const again=E.begin(s.meta,s.colony,['r2'],{food:1},'resources');
...
test('expedition state: 现代补给合并迫降舱与精确实体库存，并避开已预订食材',()=>{
  ...
  const out=E.begin(s.meta,s.colony,['r1'],{food:4},'resources',context);
...
test('expedition state: 现代补给不足失败时数字库存、实体与任务锁均零变化',()=>{
  ...
  const out=E.begin(s.meta,s.colony,['r1'],{food:2},'resources',{entities:entities});
...
test('expedition state: cargo 推进目标，reward 只描述实际回收物且不凭空加货',()=>{
  const s=fixture(),run=E.begin(s.meta,s.colony,['r1'],{food:1},'resources').run;
...
test('expedition state: JSON snapshot/restore 保留成员、货物、目标和 runtime',()=>{
  const s=fixture(),run=E.begin(s.meta,s.colony,['r1'],{food:2},'relics').run;
...
test('expedition state: 返航保留原 itemId 落地，研究物沿 settleGoods，runId 重放只结一次',()=>{
  const s=fixture(),run=E.begin(s.meta,s.colony,['r1'],{food:2},'resources').run;
...
test('expedition state: 返航只退还剩余补给且不计入 objective',()=>{
  const s=fixture(),run=E.begin(s.meta,s.colony,['r1'],{food:3},'resources').run;
  run.supply.food=1;
...
test('expedition state: 未吃完的现代精确食物按原类型返航且共享记忆只记一次',()=>{
  ...
  const run=E.begin(s.meta,s.colony,['r1'],{food:4},'samples',{entities:entities}).run;
