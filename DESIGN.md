...
### ADR-47 修订：#190 复核（2026-09-17）

工作表、征召、框选、头像条只认玩家阵营这条不变式**复核为已成立**，不需要行为改动，但「框选只收居民」此前在每个调用点各写一遍 —— 现已收口为 `Colony.boxSelectPawns`（框选 + `T.RESIDENT` 过滤）作唯一定义，漏一处即「拉框能选到敌人」。锁定用例：框选不含敌对、敌对即使塞进 `selectedRid` 也不被征召、头像条（ADR-45 名册渲染）不含袭击者、**全员被俘同样结束袭击**且俘虏不进名册。
