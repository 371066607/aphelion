/* ============================================================
   ...
   纯 Canvas 建造原型绘制：世界坐标由调用端已经处理相机变换。
   ============================================================ */
...
APH.BuildProtoDraw = (function(){
  ...
  function ground(ctx){
    ctx.fillStyle='#6f7b52'; ctx.fillRect(0,0,WORLD,WORLD);
    /* 稀疏草屑，别把地面画成满屏噪音。 */
  ...
  function furniture(ctx,bid,r,rot,powered){
  ...
  function drawObject(ctx,o,blueprint){
    ...
    furniture(ctx,o.bid,r,o.rotation,o.powered);
  }
  ...
  function drawIcon(ctx,bid,rotation,size){
    ...
    else if(/wall/.test(bid)||d.kind==='wall'){ctx.fillStyle='#d7d0c1';ctx.fillRect(-24,-24,G,G);ctx.strokeStyle='#817b70';ctx.lineWidth=5;ctx.strokeRect(-21.5,-21.5,G-5,G-5);}
