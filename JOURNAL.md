...
- **2026-08-29 · 班次**: ✅深度生存系统全链路收尾（#16, #18, #17, #19, #20，Epic #14 闭环）。
  - #16 机能损毁：`Res.capacitiesOf` 纯函数派生 Moving / Manipulation / Consciousness（0.0~1.0），分别缩放移速与工农业生产；认知作为全局上限。
  - #18 娱乐与抗压：`recreation`（0~100）衰减 5/跳，闲暇观星聚会补充；≥80 获得「身心愉悦（+8 心情）」，<20 惩罚 -5 心情。
  - #17 击倒与救援：认知<30% 或致命伤触发 `downed` 击倒（🚨），90s 濒死倒计时，就近闲人/医护背人送医疗舱用药止血；超时死亡触发全员 -8 悼念减益。
  - #19 环境暴露：极端气候室外每跳累积 +10 暴露值，建筑 48px/核心避难所消退 -15/跳，>80 转化为急性感染/外伤。
  - #20 UI 与集成：R 键名册新增精力/娱乐/暴露进度条、三维机能百分比与床位/击倒状态卡；Canvas 实体头顶渲染 💤 睡眠与 🚨 击倒图标。
  验证: `python3 build.py` 构建成功 27064KB 以 `</html>` 收尾；单元 271/0（`tests/survival.test.js` 25 用例全绿）；场景 27/0；perf 3/0；boss 7/0（全量 308 项测试 100% 绿灯）。
  下一步: 实机浏览器体验与试玩反馈。
