...
## 2026-09-07（续十一）· 方向被否：不该有指挥官

我自选了「指挥官死亡时控制权归谁」这条，做了一版**继任**方案：
指挥官倒下、最资深的殖民者接手、本局继续。跑通了，实机验证也过了。

**用户否掉了这个方向**：

> 只要有任何一个人活着都能继续，另外没有主角，都是角色，没有指挥官，要参考环世界

我选的是最保守的那个方向 —— 继任保住了主角，代价是继续背着一个 648 行的化身。
真正的答案是把化身整个拆掉：**玩家是没有身体的指挥者，所有人都是殖民者。**

我在 ADR-44 里写过一句「未采用镜头脱离指挥官方案 —— 那会砍掉所有 [E] 交互」。
**这个论证是错的**：环世界里根本没有 `[E]` 交互，那些动作是下令让殖民者去做。
而这套指挥层（选人 / `orderMove` / `orderGather` / `orderSleep` / `orderEat` /
工作优先级 / 征召 / 划区）**这个项目早就全有了** —— 指挥官是长在它上面的多余物。

**我把「现有交互挂在主角身上」当成了约束，其实它就是待拆的东西本身。**

ADR-44 已整体撤回（代码、用例、文档），不留残迹。正确方向见 ADR-45。

---

## 2026-09-07（续十二）· ADR-45：把主角拆掉

用户否掉继任方案之后的正确方向。**玩家没有化身，所有人都是殖民者。**

### 走错那一步的原因，写在我自己的 ADR 里

ADR-44 我写过：「未采用镜头脱离指挥官方案 —— 那会砍掉所有 `[E]` 交互」。

这句话是错的，而且错得很典型：**我把待拆的东西当成了不能碰的约束。**
环世界里根本没有 `[E]` 交互 —— 睡觉、吃饭、砍树、搬运不是走过去按键，
是下令让殖民者去做。而这套指挥层（选人 / `orderMove` / `orderGather` /
`orderSleep` / `orderEat` / 工作优先级 / 征召 / 划区 / 规划标记）
**这个项目早就全有了**。指挥官不是长在它旁边，是长在它上面的一层多余物。

### 拆掉了什么

指挥官自治 171 行 · `[E]/[F]` 交互处理器 366 行 · `senseHome` 的 18 个
`near*` 探测 · 生产跳里的玩家需求结算 38 行 · 击倒→送医→失血过多整条链路 ·
提示层三档 · UI 四处指挥官卡 · HUD 六条个人状态条 · `T.PLAYER` 实体本身。

### 换上了什么

**开局 3 名殖民者。** 这不是加料，是必需：没有主角之后零居民 =
什么都不会发生，没人干活、没人接受命令，连「殖民地覆灭」都无从判起。

HUD 改报人口/床位。检查器默认给殖民地概览。暴露读数从「主角的」
变成「暴露最严重的那位殖民者」—— 指标还有意义，只是主语换了。

**远征 = 派队**：优先派已征召的，一个没征召就全员出动。队员是普通的
`RESIDENT` 实体 —— 选中、下令、征召原样适用，不需要为远征另造一套操控。

### 终局差点被我删没

T5 的胜利出口原先是「走到发射器旁按 `[E]`」。拆掉 `[E]` 之后，
**整个游戏失去了唯一的胜利出口，而且不会有任何报错** ——
测试全绿，游戏照跑，只是永远赢不了。

现在它是发射器自己的一个命令（建筑检查器里的按钮），通电给、没电说明原因。

**拆一个交互层最危险的地方，不是拆坏了什么，是拆掉了某件只有一条路能到的事。**
删代码时要问的不是「这段还有人调吗」，是「这段是不是某件事的唯一入口」。

### 一个只有截图才看得见的 bug

远征绘制表 `expeditionDrawers()` 根本没有 `resident` 项 ——
以前那张图上只有化身。改成派队之后，三个队员在地图上一个都画不出来：
逻辑全对、测试全绿、就是看不见。实机截图才发现。
...
node tests/perf/boss/ui_modals → 4 / 7 / 90, 全绿
```

删掉的功能其用例一并删除（39 条场景 + 6 条 UI）—— 它们测的是不存在的交互。

实机：
```
开局:     {"playerEnt":false,"residents":[3 人],"onMap":3}
WASD:     camX 1100 → 1472   (镜头自由平移, 不跟任何人)
选中下令: {"selected":"rs_start1","order":"move"}
派队远征: {"scene":"expedition","squad":[3 人],"playerEnt":false}
console:  (无)
```

### 记一笔

**用户的一句话推翻了我一整轮工作，而且他是对的。**

值得记的不是「我做错了」，是**我为什么会选那条路**：我在权衡时把
「现有代码把交互挂在主角身上」当成了事实约束。它不是约束，它就是问题本身。
下次面对「要不要拆掉某个核心概念」，先问一句：
**我列出的代价，有多少是这个概念自己造出来的？**


## 2026-09-08 06:04 +08 · #186 房间试建 v4

- 基于 `36cc66a` 在独立工作区 `/Users/mac/aphelion-building-v4`、分支 `prototype/building-v4` 实现已批准的两房间原型。原工作区 main 仍是 `36cc66a` 且干净；正式迁移留待玩家验收。
- 从截图复现的问题是墙贴图固定单块、可见尺寸越出 48px 占格，以及中心坐标/格锚点混用。本次以单一 `footprint/cellsOf/rectOf` 约束绘制、放置、碰撞、路径、电力，墙四邻接共享填充，四向家具贴格摆放。
- 新增纯模型、Canvas 绘制与模式控制器；精确 query 在 main 整个初始化之前分流。双房间含三床/病床/餐桌椅/科研厨房工作台/货架/外部发电机，三人真实走到预订互动格执行命令；蓝图由空闲居民施工，保留施工占人格保护。
- 墙门围合 flood fill 与地板独立；自动屋顶默认隐藏，合并/分割继承温度，雨冷热场景反馈室内外效率。建造/地板/导线按层拆除；鼠标与键盘均可选择、放置和下令。
- 双轴评审修正：补纯键盘家具选择、stop/boot 清理生命周期、完整格边界（消除沿地图半格绕墙），并在最终交付前重建 game.html。另实际 Canvas 检查修正旋转变换、可见范围、分数缩放细缝和不可见的地板/导线蓝图。
- 验证：原游戏 828 单测 / 112 场景 / 4 性能 / 7 Boss / 94 UI 断言通过；原型 17 个模型场景、精确入口探针、按 build 顺序加载所有模块的 DOM/键盘/重启探针通过，存档访问计数为 0。离线原生 Canvas 对 13 类 × 4 方向实际世界绘制/图标边界及墙缝、蓝图可见性检查通过。98 格地板规划 66ms，首次派工 89ms，后续 200 tick 共 259ms（本机一次测量）。
- 证据与操作入口：`docs/prototypes/building-v4.md`；画面 `docs/prototypes/evidence/building-v4-canvas.png` 为真实 drawWorld 离线输出。Canvas 取证依赖只装在 /tmp，不是项目运行依赖。
- 验证边界：浏览器本地 URL 自动访问被安全策略拒绝，未通过其他地址、浏览器或服务器绕过；浏览器实机手感尚待玩家验收。下一步是玩家试建、确认方向，再正式拆票；#186 保持验收待办，不宣称正式系统迁移完成。

### 2026-09-08 · #186 玩家要求直接合并 main

玩家在原型实现及验证后明确要求直接合并到主分支。已将 `prototype/building-v4` 的 `f3f78e5` 快进合并到 main，无冲突；代码与已通过全部验证的提交完全一致。同步入口文档为 `/Users/mac/aphelion/game.html?prototype=building-v4`，保留模式隔离与实机验收待办。此前两次原型分支推送遇到 GitHub HTTP 408/连接中断，后续直接推送 main，远端结果以推送验证为准。

### 2026-09-08 · #186 美术方向确认与首张样稿

玩家通过 ask-matt 访谈接受“艰难但温暖：迫降后逐渐重建家园”的方向。以现有开场第五张插画为材质参考，以双房间原型为布局参考，使用内置 image_gen 生成首张手绘科幻样稿，保存于 `docs/prototypes/evidence/building-v4-warm-concept-v1.png`；完整提示词与方向记录一并保存。目视核对双房间、三床、病床、三人、门及工作设施布局保留。图稿是视觉提案，尚未通过玩家观感验收，也未接入游戏；此轮无源码/正式贴图改动，不重跑业务测试。下一步根据玩家对样稿的反馈确定观感，再处理逐格资产拆分与接缝适配。

### 2026-09-12 12:12 +08 · #186 暖色试建美术落地与普通墙修复

- 按玩家“要做就别停”及“继续”要求，在 main 接入暖色迫降殖民地图集：回收舱板、旧科研/厨房/维修设备、织物床褥、暖灯餐桌与土色地形，UI 同步为暖炭灰舱内面板。原型三人、双房间、建造/旋转/预约/寻路/拆除与分层模型不变；菜单图标、预览、实体共用占格绘制。
- 内置 image_gen 产出图集；透明底修正一次超时、一次仍输出 RGB 棋盘底，最终沿项目 ADR-11 的 PIL 管线复用 `_clean_cell`，为该图集追加中性灰边缘清理和 2 源像素去边。原稿、成品、提示词均保留；不改既有 `sprite_data.js`。Pillow 12.3.0 仅装于 `/tmp/aphelion-building-art-tools`，正常 `build.py` 使用标准库校验 RGBA 成品并自动内联，无新增游戏运行依赖。
- 普通入口只改 `entities.drawWalls`：墙门以真实中心坐标连通填充、只描外露边；取消巨型等轴块、逐格内框和色带。保留伤条/裂纹、原闭门表现与世界昼夜暗幕，不造开门状态字段，不迁移普通家具或存档。`main.js`、原型模型、人物数据、居民及存档源码与本轮基线一致。
- Standards 独立评审发现的逐格 fallback 内框和构建可误包 RGB 原稿两项已修，复核通过。构建门禁拒绝原始 RGB 图集，原生 Canvas 另验证真实透明通道；墙视觉测试覆盖横/竖/L/T/十字/墙门、偏移中心、非整数坐标、内部无线与受损态。
- 主代理统一验证：构建成功（game.html 40691KB）；843 单测 / 112 场景 / 4 性能 / 7 Boss / 94 UI 断言通过；原型 17 模型场景、精确入口隔离与 DOM/键盘/重启探针通过。真实 Canvas 回退和图集各 13 类 × 4 方向的实体/图标占格、蓝图可见与非整数缩放接缝通过；33 个当前源码模块逐字内联一致。
- [实际离线绘制](docs/prototypes/evidence/building-v4-warm-runtime.png) 使用真实模型、人物与图集，区别于生图概念稿。浏览器本地 URL 的安全策略禁令持续遵守，未经其他浏览器/服务/地址绕过；浏览器视觉及操作手感仍待玩家验收。#186 保持验收待办，正式建造业务迁移没有冒充完成。
- 最终 Spec/视觉评审通过：双房、三人、家具比例及已批准材质方向可读；门图集的方形感比概念稿更强，属可辨识的视觉细节，非阻断。评审提到的 JOURNAL 缺口在并行收尾时已补齐；未将 DOM/离线证据升级为浏览器实机结论。

### 2026-09-12 14:00 +08 · #164 正式异星家园第一批实现（五阶段 WIP）

- 按用户完整方案落下 `docs/colony-home-plan.md` 与 ADR-46 修订，并行分工几何、地形、物流，主代理整合正式入口；初始 main 工作区干净，未覆盖他人 WIP。本轮未提交、推送或关闭 issue，不能把五阶段方案标为整体完成。
- 正式 BuildGrid 统一新家具格坐标/旋转/占格/点击/预览/互动格，床1×2、桌2×2、部分工位2×1，墙门分层连续；旧布局保留原像素中心与旧占格。房间和导航缓存随结构失效，房温重算按重叠面积继承。基础墙门床地板货架与篝火可开局规划。
- 建造物流接入正常居民循环：缺料可规划，指定树采集→地上堆→预订/取料→运至工地→合法互动格施工→独立床睡眠。实际到场才施工，取消/失能/堵路释放认领，读档恢复在途任务；孤儿任务返还、复合物资超额量进入 durable pendingGround 再落地。所有物理配方工位物流仍待实现，迫降舱数字库存是现阶段兼容接口。
- 新档128×128格、六区域、约2000自然资源对象、两块20×20建地；水域影响建设通行，湖岸和矿丘肥力参与农田/种植圃/新划种植区。植被恢复保留来源，矿点采空持久化，清理现代地图遗留矿物再生队列。贴图按需LRU最多36块；仍无地图缩放/总览、全实体空间查询、真实野生生态与残骸修复。
- 家园 v2 原子快照同时保存 metaSnapshot、stock、地面物资、在途任务与地形耗尽；Save.loadMeta 从整份家园快照恢复，避免旧 colony.stock 覆盖刚扣过的库存。旧档保持2200像素、UID迁移、旧蓝图已付材料与旧居住舱语义，PlanetSpec 仍v1。
- 部分岗位到工位才生产，检查器给缺工/断电/堵口原因；首日袭击保护、60秒预警、损失恢复窗口；灯塔保留旧 ID，改成连续三日扎根进度，达成后继续沙盒。扎根当前条件使用种植+口粮缓冲/设施状态，完整稳定供应、真实冷库存粮和异星作物已利用仍需深化。
- 独立关键审查发现并修复：meta-only 消费刷新复制、漏收 costMineral、3×3 灯塔合法工位距离超过旧90半径而永不完工、storeN超额送达吞余料。统一修复正常AI移动后再沿旧tx/ty走一次的折返问题，保留闲逛单次移动。审查复核限定范围无剩余阻断。
- 最终统一验证：`python3 build.py` 成功（game.html 40765KB）；890 单元 / 113 scenario / 5 perf / 7 Boss / 95 UI 断言全绿；原型17模型用例、精确入口存储隔离、DOM/键盘控制回归通过；diff-check通过。
- 大图性能夹具：128格、20居民、2000自然对象，DOM桩模拟 P50=0ms、P95=2ms（Date.now毫秒精度），20次寻路及保存计时3ms，存档126358字节。不是实际渲染帧率，不冒充20居民实机性能验收。
- 真实无头Chrome：`node tests/home_live_probe.js /tmp/aphelion-home-qa` 使用独立临时profile，不读用户浏览器数据；正式 game.html?autostart=1（仅跳开场，无debugmark赠送建筑），家园6144px、3人、0建筑、0虚构床位、2067对象。独立无限建造夹具14项全部落成（四朝向床、墙门、桌椅），无捕获运行时异常。查看 ordinary-home.png 与 four-direction-furniture-fixture.png，确认连续轮廓及格内暖色家具。该自动截图不代替人工操作手感，也不证明生存长链路。
- 下一批仍须做：完全容器化与生产物流；双世界同时仿真/队员补给选择/三种任务/跨重启返航事务/记忆；缩放总览/动物生态/可修复残骸/全房间用途；完整一季、可补救危机、五分钟留守生产、实际浏览器大规模帧性能与人工体验验收。

### 2026-09-12 16:10 +08 · #164 正式异星家园续作与集成验收（工作区实现，未发布）

- 将实体仓储、工位配方、燃料运输、双世界仿真、队员补给选择、三类远征、地图缩放总览、野生生态、异星标本、可修复残骸、生活空间与扎根条件接入普通入口。旧地图/几何/已付蓝图和迫降舱库存兼容保留；不改医疗模型和 API 配置。
- 物资守恒覆盖：跨资源别名预约、普通搬运重量上限、吃饭/装备/陷阱不抢预约物资、在途读档、失能及移除掉货、配方切换退料、拆除实际材料与未燃木材退款、双世界掉落 ID 避碰、来源 ID+物品类型+位置校验。
- 正式远征无幽灵主角：真实居民接受移动/采集/战斗命令，留守居民继续工作与需求结算；原子快照保存两世界及货物，返航收据保证幂等。资源任务现有有限铁石矿脉；实际采铁→拾取→快照→返航实体铁堆的独立场景通过。样本研究回流与共享记忆接入。
- 连续季节测试发现并修复真实运行缺陷：现代导线错误使用旧中心取格、低血逃跑敌人未进入撤退回收导致袭击永久不结束、竖直墙缺口施工格被邻墙阻断。独立物流复核再修：现代同格分层导线不并网、配方退款掉进阻挡工位中心。各有行为回归，后两项独立复核通过。
- 主代理最终集成检查：构建40895KB；988单元、122场景、5性能、7Boss、95UI、17原型模型以及原型boot/UI全部通过；独立关键场景4项通过；diff-check通过。自动生成 game.html，未手改产物。
- 真实 Chrome 152 独立profile：普通新档6144px、三人、零建筑/零虚构床位；50%/200%鼠标放置、总览定位不挪居民、队员选择移动、远征重载与重复返航通过，无捕获异常。证据见 `docs/evidence/colony-home/`，图像夹具与普通入口分别标注。大图性能记录覆盖128格、20居民、超过2000对象、120真实RAF帧，包含CPU、间隔、20次寻路、缓存和存档大小；不以DOM桩性能代替浏览器测量。
- 本条写入时六日连续长测仍在运行，单独补充结果，不将跳日条件夹具冒充生存游玩。一季人工完整游玩、建造手感、地图密度和长期平衡仍待用户本地体验；未提交、推送、发布或关闭 issue。
- 时间校正：上一条标题的 `16:10 +08` 为记录笔误；写入后工具核对实际时间是 `2026-09-12 15:58 +08`。保留原条目并追加更正。

### 2026-09-12 16:03 +08 · 季节长测驱动的战时后勤修复

- 六日长测首次在修撤退/修墙后跑至8370/21600模拟秒仍失败：房间存在但四台木柴机无燃料，暖气/医疗断电，居民低温、饥饿和感染。仓储仍有木材；原 main 全局 `!raid` 门控让受保护室内居民也永久停止取餐和补燃，不能靠新增驻军掩盖后勤契约冲突。
- 现代未征召居民在完整房间且同室无活跃敌人时放行生活/后勤；敌人入室、户外及旧家园保留原避险。新增独立真实场景：敌人在室内时不认领物流，离开房间后居民实际取走8木并送入空发电机，库存16→8、机仓0→8。关键场景增至5项全绿，122场景与构建重新通过。
- 长测按相同合法预建基地重新运行，不补充运行中库存，不强制房温/供电，不增加作弊防御；最终结果继续单独追加。

### 2026-09-12 16:35 +08 · 取餐与施工短场景闭环

- 不可达近处口粮会压过可达远处食物：新增回归先红后绿，现代 `nearestMeal` 按熟食/距离优先后选择实际寻路可达来源。季节夹具初始食物改由 Storage.deposit 放入合法货架入口，避免把裸掉落塞进家具实体中心。989单元、122场景重新通过。
- 修正后的六日运行已实际完成21600秒，三名原居民存活且未倒地、真实采铁返航5单位、300秒双世界并行、粮荒恢复、708次补燃；该次命令仍exit1，原因是随机战况无俘虏却被断言要求必须俘虏，随后检查也发现墙损重建未完成。未把失败退出写成全绿，原始失败数据随后会被最终同入口测试报告替换。
- 将修墙缩为独立真实短场景，定位旧 `isBuilder` 专长/等级≥3门槛：普通居民已把4石送齐、站在合法互动格、显示施工动作，实际进度却被资格过滤为零。现代规则只取消这个隐藏门槛，保留岗位/优先级/失能/征召限制及legacy资格。显式技能1、非建造专长居民现在实际送料补墙并恢复房间；独立复核通过，关键场景增至6项全绿。
- 季节测试不再硬要求随机袭击必须产生俘虏；有实际墙损才要求重建。补燃证明改为观察相邻帧机仓增长，报告不把运输中的木材误算为燃烧；`passed` 只有全部断言通过才写true。固定夹具随机源9301，完整六日重跑进行中。

### 2026-09-12 16:49 +08 · 正式异星家园自动验收收尾

- 固定种子9301的完整六日测试最终exit=0：144000个正常帧步、21600模拟秒，三人健康留在家园，主生活区21℃；真实资源远征300秒、返航5铁矿，粮荒补救与708次实物补燃成立。强攻通过正常征召、移动及俘虏命令处理，随后恢复生活和生产。没有运行中添库存、强制回血或强制供电。
- 该长测使用合法预建基地和一季初始燃料储备；远征段隔离随机战斗、家园袭击保留。本次强攻没有毁墙，不能把条件断言当成实际重建；毁墙→低技能居民送料施工→房间恢复由独立关键场景第6项实际验证。失败历史样本及最终passed报告分别保留在 docs/evidence/colony-home/。
- 最终构建40896KB；989单元、122场景、6独立关键场景、5性能、7Boss、95UI、17原型模型及boot/UI通过。最终Chrome相机与远征重载探针通过，无捕获异常；实际128格/20居民/2000以上对象性能：CPU帧P50 2.7ms、P95 5.9ms，20次寻路52.60000014305115ms，存档743481字节。
- 自动实现与验收证据已落仓库。仍待人工从普通入口零起步完整游玩、建设手感、地图密度和长期平衡确认；不宣称正式对外版本已发布。本轮未提交、推送或关闭issue。

### 2026-09-12 22:39 +08 · 全仓双轴审查后的安全修复

- 按审查落地：Boss 击杀只记 `guardianCleared`、灯塔目标改扎根文案、新家园修理取消技能≥3 门槛、新餐桌/工位目录格对齐 SHAPES 且旧实例保留 LEGACY 占格、植被再生按 scene 夹边界、动物 HP/`HOME_SIZE` 进 CFG、layering 名单补新模块。框选不再塞假指挥官；无远征 run 不再走 `updatePlayer`。未改全局 `CFG.WORLD`。
- 验证：`python3 build.py` 成功（40898KB）；993 单元 / 122 场景 / 5 性能 / 7 Boss / 95 UI 通过。未提交、未推送。普通入口一季实机仍待人工。

### 2026-09-12 22:50 +08 · 命令抽屉挡住画布 POINTER_DOWN

- 打开底部「命令」会 `pushContext('modal:orders')`，ADR-33 因此丢掉 pointerdown，规划拉框和框选小人无法开始。全屏 overlay 才占 modal 上下文；抽屉保持 game。
- `node tests/ui_modals.test.js` 98 通过（含命令抽屉仍能 POINTER_DOWN）；构建成功。

### 2026-09-12 23:10 +08 · ADR-47 人型只有一种对象

- 用户拍板：人型袭击者是另一阵营的小人，需求与念头都有；减法在工作树不在人格。写入 `docs/adr/0035-humanlike-pawns-are-one-type.md`，DESIGN.md / CONTEXT.md / 环世界对齐表同步。未实施代码，当前袭击者仍是战斗壳。

### 2026-09-13 00:50 +08 · #188 人型袭击者走小人缝（工作区，未单独提交）

- `Res.hostilePawn` / `embodyHostile` / `isHumanlike`：家园袭击刷人（名字、需求、念头），基因团 `makeEnemy` 仍非人。点人只检查不征召。998 单元绿。未 commit：工作区还有家园 WIP。

### 2026-09-13 01:10 +08 · #189 俘虏/招降同一 id（工作区）

- `capturePrisoner` 人型不标死、不另造桩；`recruitPrisoner` 改阵营进名册。袭击计数排除 `captured`。过客招募本就保 id。1001 单元绿。未单独提交。
...
### 2026-09-13 · feat/adr-48-observe · #194

- 走/肥/岸/水读观测格；初局树矿来自砖；再生同格同种；无水酸化熄火；遗迹落可走格。未观测查询仍为空走旧撒点。单元 855 绿。
...
### 2026-09-14 05:20 +08 · #203 远征资源与覆盖物闭环

- 新 observed 星球第一次观测就把群系自然资源写入 `Observation.resources`；运行世界只从这份清单实体化，不再叠加旧 rock/crystal/flora。共享资源表统一可见类型、产物、数量、耐久、耗尽、seed 与再生周期；旧 v1 最小记录按 seed/坐标补齐新增字段，显式冲突、重复 uid/坐标和未知 kind 则安全拒绝，不再默认掉木材。四类群系的保底植物都可推进“植物取样”。
- 有限矿物采空后保存明确耗尽态；可再生植物随 Active Run 保存队列，并以同一 uid、格位、可见类型、产物、数量和 seed 恢复。generation 0 继续保留一次点击取得旧标本且不运行新再生时钟。损坏或超大的远征再生相位在存取边界归一，Active Run 恢复门禁拒绝危险值，帧循环补算次数有上限。
- 遗迹 5×5、敌方基地 2×2、任务矿点和守军 1×1 都用各自 seed salt 做有限候选加有限全图扫描，逐格避开水、边界、着陆区、信标、自然资源及先前占地；无合法位置记录显式 failure，不强放。1000 个 seed 探针中 PlanetSpec 非法 0、资源重复 0、守军占地冲突 0；2 次覆盖物无合法位置均走预期显式失败。
- 最终构建 `game.html` 41976625 字节，SHA-256 `86cf07a7c03bbee9ea0df6b06ed7536d29871f9e95a32bf315617bb398818dc8`。1059 单元、142 scenario、5 perf、7 boss 全绿，`git diff --check` 通过；DOM 性能桩为 P50=1ms/P95=3ms、20 次寻路84ms、存档1610809字节，只代表测试桩。场景测试覆盖实际着陆、开采、checkpoint/刷新、带货返航、新 Run 重置、旧地图语义和损坏时钟召回；最终产物的普通入口人工新档验收留在 #204，未把自动场景称为人工游玩。

### 2026-09-14 05:28 +08 · #203 实体身份隔离与最终冻结证据

- Standards 复核发现 Observation 资源可伪造 `rw_0`，进入统一实体表后会覆盖远古遗迹墙的恢复索引。远征自然资源的显式 uid 现必须精确等于 `obs_<worldSeed>_<kind>_<gx>_<gy>`；早期 v1 最小记录仍可缺省并按相同规则补齐。PlanetSpec 校验和绕过校验的运行时实体化都会拒绝跨命名空间身份，损坏 Active Run 沿既有幂等路径安全返航。
- 新增跨自然资源/任务矿点/遗迹/敌方基地的实体 ID 唯一性与采集引用存档往返回归，并增加恶意 `rw_0` 星球+runtime 刷新场景。正常引用恢复为原 flora，不会解析成 `ancient_wall`；恶意档不安装。此前 pre-#203 observed runtime 缺少资源闭环标记的场景也保持安全返航。
- 重建 `game.html` 41977218 字节，SHA-256 `fa95d9d85951b793b4f0929bfaced3de5137dc25eb8994d2f56a58ffbb3d2628`。1060 单元、144 scenario、5 perf、7 boss 全绿，`git diff --check` 通过；DOM 性能桩 P50=1ms/P95=3ms、20 次寻路79ms、存档1610685字节。该条取代上条修复前的构建指纹与测试计数，人工新档验收仍归 #204。

### 2026-09-14 05:33 +08 · #203 显式空身份边界与重新冻结
...
### 2026-09-14 06:33 +08 · #204 地图观测全量回归与正式验收

- 远征标题统一由 PlanetSpec 的名称和群系生成；LLM 富化只有在完整 PlanetSpec 保存成功后才同步当前 `name/paletteName/lore` 并刷新标题。新增场景回归覆盖富化后的即时标题与整页恢复标题一致，已知星首次着陆、刷新和重访也保留同一身份。
- 隔离 Chrome 性能探针固定 seed 58098、128×128、20 居民、2059 世界对象。三轮 frame CPU p95 为 7.4/6.2/6.5ms，20 次 A* 为 73.5/71.7/71.1ms；主记录 1,090,398 字节，其中 Observation envelope 精确增加 398,208 字节，去除后 692,190 字节，比历史基线低 6.90%，`homeRuntime` 无第二份 Observation。三轮 Runtime/console/Log 错误均为 0。
- 自动双世界探针用正式 planner DOM `click()` 提交和纯键盘重访，访问次数 1→2、PlanetSpec 字节不变、刷新/幂等返航/强制重选均通过；它的移动和 2 件货物是显式夹具。普通入口另用全新 origin 和无查询参数新档实操：小满·三号移动、耗 1 木铺地板、首次着陆 `APH-P9CDE8818 · 酸蚀巨沼星`、实际搬运得到 `+2 晶体矿`（任务 2/6、负重 6、异常 0/6）、刷新保持、返航后重访同星并开启 0/6 新 Run。人工流程没有存档编辑或状态注入。
- 最终构建 `game.html` 41977626 字节，SHA-256 `19b8d718dbeb54ad73b26b464833545c73941b24bb63fce28d6000e13f5441fb`；1060 单元、145 scenario、5 perf、7 boss 全绿，`git diff --check` 通过。人工页仅有导航/刷新各一条无游戏源码栈的浏览器扩展消息通道关闭日志；证据、复现命令和输入指纹收口在 `docs/evidence/map-observation/`。
- Spec 与 Standards 最终复审均 PASS。审查者独立重建得到相同字节与 SHA，复跑完整门禁、双世界 Chrome 链路及固定 seed 性能证据，未发现剩余 blocker；#204 未触碰 illness/#64，远端祖先关系允许普通快进推送。

### 2026-09-16 12:10 +08 · #205 寻路 open 集换二叉堆（单次 2.9ms → 0.3ms）

- `APH.Nav.astar` 的 open 集从「普通数组 + 每轮线性扫描最小 f + `splice` 取出 + 每个邻居 `Array.find` 闭包查重」换成二叉堆（懒删除）+ 整数节点索引 `y*cols+x` + `gScore/prevOf/closed` 三个 TypedArray；起点被墙覆盖的虚拟起点由索引 `total` 承载，堆用模块级 scratch 容量翻倍增长。对外语义逐项不变：越界拒绝、直线退化单点、目标在墙内 null、起点逃生的第 0 列约定、对角禁穿角、`costFn` 罚权 + 地形倍率、终点精确坐标。`costFn` 全程是纯查表（combat.js 陷阱罚权），不重入。
- 等价性用可复跑探针钉死：新增 `tests/nav_equivalence_probe.js`（默认把 `git show HEAD:src/nav.js` 当改写前基线）。184 例随机语料（墙密度 0/0.12/0.28、地形造价表、陷阱 costFn、真实家园 seed 58098）下 null 判定/最优代价/终点坐标差异 0、新实现非法路径 0；另与该文件内独立实现的 Dijkstra 逐例比对 135 例全为最优（跳过 19 例直线退化与 8 例起终点本身被墙覆盖的退化约定）。`tests/nav.test.js` 补 #205 手推最优用例：60×60 + x=30 单列墙只剩 (30,55) 缺口，最优 = `102 + 48(√2-1)`（缺口格上下邻居是墙 → 进出它的对角步被防贴角穿墙禁掉，只能正交进出）；该用例在改写前后两份实现上都通过，锁的是契约不是实现。
- 隔离 Chrome 实测（固定 seed 58098、128×128、2059 世界对象）20 次 A*：改写前 73.5/71.7/71.1ms → 改写后 12.0/10.9/10.8ms，中位数 71.7 → 10.9ms（约 6.6×），逐条路径长度与改写前完全一致。帧 CPU p50/p95 为 3.4/6.6、2.6/6.3、2.6/6.1ms，与改写前 3.1/7.4、5.5/6.2、5.5/6.5ms 同量级波动——帧成本由渲染主导，本次消除的是「一次多人同时重算寻路」的成帧尖峰（旧实现 20 条 71ms 超过一个 16.7ms 帧预算的 4 倍）。三类浏览器错误均为 0。
- DOM 桩 `tests/perf.test.js` 同 harness 对照（基线副本只替换 `src/nav.js`）：`simulationP95Ms` 12 → 2ms、`navigation20Ms` 109 → 36ms。证据收口在 `docs/evidence/nav-heap/`（四次探针报告 + 差分输出 + 复现命令），ADR-13 文档补实现追记（正文的 46×46 尺度已过时，现代地图 128×128）。
- 构建 `game.html` 41,979,666 字节，SHA-256 `adfb40115a0d82fdd31ac51e1a18f0b87b02197ff45c5f2e4653bd01d7603ab5`（连续两次构建字节一致，三次探针都跑在这份字节上）；1061 单元、145 scenario、5 perf、7 boss 全绿，`git diff --check` 通过。

### 2026-09-16 13:05 +08 · #206 main.js 拆分第三批：居民世界侧接线独立成 APH.ResidentWork

- 先量后切：用调用图脚本量 `updateResidents` 的依赖闭包（28 个函数、主体 559 行 + 27 个 helper），发现 20 个只被本簇调用（375 行）可整体搬走；7 个簇外也有调用者中，`residentOf`/`nearestMeal`/`nibblePile`/`foodEatBelow` 只是转发已被归位的领域函数（21 处调用点改直连后删壳），真正要缝的只有 `freeDropCount`/`nearestDrop`/`syncResidentEntities`。被搬代码引用的 main 模块作用域符号只有 `CFG/U/T` 三个。`updateCmdPanel`/`updateInspectorNow` 实测本簇零调用（帧循环每帧已在刷），因此不做缝。
- 顺带扫出死代码：`doHaul`（39 行）全项目零调用点，其四条搬运原语已被 `updateResidents` 内的等价路径取代，只剩注释里一句「同 doHaul 逻辑」。分两片落地：`59d787a`（删死代码 + 内联薄包装，main 4730 → 4686），本片新建 `src/resident_work.js`（`APH.ResidentWork`，1045 行）并搬走 24 个函数、1009 行，main 4686 → 3675（累计 ADR-41/42/43/49：6048 → 3675，−2373）。
