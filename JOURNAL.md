...
## 2026-09-07（续十一）· ADR-44：指挥官倒下，有人接班

用户让我自己挑一件做。挑了挂最久、也最该先定的那条 ——
「指挥官死亡时控制权归谁」。

### 先把现状数清楚

全项目「本局结束」有四个出口：远征战死、远征缺氧、家园失血过多、殖民地覆灭。
**四分之三是「指挥官之死」。** 而 DESIGN 里「殖民地优先」写着不可动摇。

规则和支柱直接打架，且打架的是规则那边。这条其实不需要什么权衡 ——
**决定写在 DESIGN 里已经很久了，只是代码没跟上。**

（顺带澄清：家园其实已经不会直接死 —— `hp<=0 && scene==='home'` 走的是击倒，
只有无人救援倒计时归零才死。争议面比 BACKLOG 描述的窄。）

### 为什么不是「镜头脱离指挥官」

那是另一个自洽答案（ADR-29 本来就是上帝视角），但代价太大：
`[E]` 交互 —— 睡觉、吃饭、招募、切菜谱、破译终端、登船 —— 全挂在指挥官身上。
没有身体就没有这些动作，等于砍掉半个游戏。

继任不是妥协，是把设计意图讲成了故事：**殖民地是主角，个人不是。**
你会失去跟了你三十天的那个人，然后另一个人走过来接手。

### 代价必须是真的，否则「死亡」就没有重量

- 名册 −1（接班的人从劳动力变成指挥官）
- 继任者**带着自己的饱食/精力/心情/病情上任** —— 他刚经历同一场灾难，
  不是满状态复活（生命值是新身体，所以回满）
- 前任的技能没了，谈判和破译从此按继任者的水平算
- 死在远征：背包与本趟收益随人留在荒原，镜头回家
- 死在家园：留下尸体

远征的风险因此从「可能直接输」变成「可能赔上一个人和一趟收益」——
反而更贴 T4「远征是补给不是主线」。

### 指挥官第一次有了身份

此前全项目写死 `{ id:'player', name:'指挥官' }` ——
**因为他不可能被替换，所以不需要身份。** 继任落地后他需要。

初代**没有技能表**，`commanderSkill(meta, key, fallback)` 回退到各调用点
原先写死的值 —— 所以初代行为一字未变，只有继任者开始用自己的真实技能。
老档补一位无名初代，零迁移。

### 一个订阅点的位置差点埋雷

`combat.js` 加载早于 `colonytick.js`，够不着继任函数，所以走事件。
我一开始把订阅写在 `main.js` —— 结果 `tests/run.js`（不装 main）里
**玩家干脆死不掉，而用例还是绿的**。

挪进 `colonytick.js` 自己（它就在 combat 之后）。
教训：**订阅点放在哪，决定了哪些环境下这条链路是通的。**
放在最外层看着"合理"，实际是把链路的完整性押在"调用方记得装 main"上。

### 顺手发现一条空转的旧用例

`O2 死亡` 那条打桩 `APH.UI.showDeath` 来断言。ADR-40 把死亡结算改走总线之后，
**打桩公开方法已经拦不住了** —— 它一直在测一个不再存在的调用路径。
改成订 `death` 事件，测契约而不是内部调用。
...
截图确认交接卡、检查器改名（且饱食是继任者自己的 93 而非满值）、
地上前任的尸体、头像条只剩一人。
...
node tests/scenario.js       → 162 通过 / 0 失败
node tests/perf/boss/ui_modals → 4 / 7 / 89, 全绿
```

### 记一笔

**这条挂了很久，是因为它被当成「需要拍板的设计题」。
但真正数完四个出口之后会发现，它是一道「代码没跟上已有决定」的题。**
DESIGN 早就写死了殖民地优先，只是没人去数还有几处规则在说别的。

下次遇到「悬而未决」的条目，先做一件事：**把它涉及的现状全部数出来。**
数完之后，有些"设计争议"会自己消失。
