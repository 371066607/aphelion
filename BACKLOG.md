...
- [ ] 天气预报 HUD：显示「明天是 X」——天气 v1 Out of Scope 明写留进阶；markov 转移表已有, 只需按当前天气+转移概率推明日最可能并显示
- [ ] 玩家 exposure 条：天气 v1 显式裁剪（当时五条已忙）——如今补齐第六条（酸雨/寒潮/雷暴室外累积, 回家/房间消退；玩家不累积伤病只减速+视觉）

### P2 战争纵深（小→中, 2 项）

- [ ] 敌人会躲陷阱：**陷阱只对"无路可走"的追逐成立**——A* 路径里陷阱格加代价惩罚（绕行优先），仅当绕无可绕时踩；沙袋不绕（减速是软掩体语义）
- [ ] 围攻工兵拆陷阱：靠近的工兵优先拆掉挡路陷阱（与破墙同决策树, 陷阱耐久=1 次）

### P3 生活家具（中, RimWorld 房间幸福度方向）

- [ ] 家具三件套：电视/书架/地毯（可建造, 科技挂靠机械锻造；房间内摆放给心情增益, 数值进 CFG）
- [ ] 房间幸福度聚合：房间心情 = 卧室级(含居住舱) + 家具加成（复用 T9 roomMoodGain 语义扩展）
