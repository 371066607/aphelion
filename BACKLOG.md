...
- [ ] **提示优先级存疑两处**（ADR-41 搬出提示层后才看清，用例已按现状钉死，
      ...
      被「食物将尽」盖住，玩家看不到该怎么办
- [x] **#205 寻路 open 集换二叉堆**：`Nav.astar` 的 open 集从「数组线性扫描 + `splice`
      + `Array.find` 查重」换成二叉堆 + 整数节点索引 + TypedArray，对外语义逐项不变
      （184 例随机语料新旧差分 0 差异、135 例对照独立 Dijkstra 全最优）。
      隔离 Chrome 实测 20 次 A* 71.7 → 10.65ms（约 6.7×，逐条路径长度一致），
      DOM 桩 `simulationP95Ms` 12 → 2。证据 `docs/evidence/nav-heap/`，
      可复跑探针 `tests/nav_equivalence_probe.js`
...
- [ ] 玩家实机视觉与操作手感验收。试建保留作回归参照；正式迁移已按2026-09-12完整家园方案另外实施，见下节。

## 正式异星家园（2026-09-12，五阶段仍未整体完成）

- [x] 正式 BuildGrid、四向家具/连续墙门、分层放置与点击、房间和导航结构缓存、房温继承；新建独立床，保留旧几何和旧居住舱。
- [x] 建造物流首条闭环：指定采集→地面物资→预订取料→运到工地→合法互动格施工→入住；缺料规划、取消/失能/堵路释放、孤儿恢复和超额余料守恒。
- [x] 新档128格六区域地形、约2000自然对象、起始补给与建地、多种子确定性、有限矿点、植被恢复；地形影响建造通行与种植肥力，地形块LRU缓存。
- [x] 旧档2200像素及原坐标兼容，家园原子快照一起保存名册库存/地上堆/在途材料，避免跨key库存回滚复制。
- [x] 部分岗位到场生产、等待原因、首日袭击保护/60秒预警/损失恢复窗口，通信灯塔和连续三日扎根进度；完成进度后继续经营。
- [x] 新链路及独立审查修复回归；普通新档和四方向家具真实无头Chrome Canvas检查。
- [x] 实体仓储、工位配方取送、燃料补给、重量容量与跨别名预约；旧数字库存固定映射迫降舱。并发、换配方、取消、失能、读档和拆除退款有守恒回归。
- [x] 双世界同时仿真与原子持久化、队员/补给显式选择、三类远征、跨重启幂等返航与共同记忆；实际矿石采集与返航接入。
- [x] 地图缩放/总览、地形移动代价、实体视域索引、野生动物、可修复残骸与异星区域标本。
- [x] 到床休息、取餐后到餐椅用餐、到场工位、房间温控与保鲜；扎根要求实际冷库存粮和异星作物收获。
- [x] 五分钟双世界场景、真实 Chrome 地图鼠标操作及远征重载回流、大地图20居民2000对象真实帧测量。
- [x] 固定种子预建基地六日连续生存长测：144000步、实际采矿返航、粮荒补救、补燃与正常指挥防守，exit=0；低技能居民真实毁墙重建另有短场景验收。
- [ ] 人工从普通入口完整游玩一季，确认建设手感、地图密度、危机补救和长期平衡。预建基地自动夹具不能替代这项人工体验验收。

## 地图观测修复（#197，2026-09-14）

- [x] #198 建立复合 Observation 兼容扩展缝：一个 Ground Tile + 至多一个带产物语义的初局自然资源；有限重试、确定性降级、群系邻接/资源兼容表与 TerrainModel 查询入口。
- [x] #199 完成新家园观测与存档迁移。
- [x] #200 让家园地图成为唯一事实源：世界块/总览、玩家碰撞、A*、种植肥力、生态与自然资源均读取 Observation；generation 0 保留旧规则。
- [x] #201 打通未知目的地远征：原生目的地选择只在提交时生成；PlanetSpec/Atlas 先落盘，Active Run 保存星球身份并创建/恢复 observed WorldRuntime；正式与调试入口同链路。
- [x] #202 完成已知星球重访与故障恢复：Atlas 列出并显式重选已知星；重访读取原 PlanetSpec、原子记录访问且不改地图字节；刷新恢复 observed/legacy Active Run，失败不扣补给不切世界。
- [x] #203 完成远征资源与覆盖物闭环：observed 自然资源只由 Observation 实体化，采集/耗尽/再生/刷新保持身份；覆盖物与守军按完整占格确定放置，损坏语义和无合法落点显式失败。
- [x] #204 全量回归与正式验收。
