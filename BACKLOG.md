...
- [x] ~~**指挥官死亡时控制权归谁**~~ → **ADR-44 已定：有人接班**。
      指挥官倒下时只要还有殖民者，最资深的那位接过指挥权，本局继续；
      名册空了才是结束。**「本局结束」现在只剩殖民地覆灭一个出口**，
      规则终于和「殖民地优先」一致。代价是真的：名册 −1、继任者带自己的
      身体状况上任、远征阵亡赔上背包与本趟收益、家园阵亡留尸体。
      未采用「镜头脱离指挥官」方案 —— 那会砍掉所有 `[E]` 交互
- [ ] **结算页展示历任指挥官**：`stats.commandersLost` 已在记，但通关/覆灭页
      还没讲这一局换过几任、都是谁。ADR-44 的叙事线只做了一半
- [ ] **继承人可否由玩家指定**：现在固定「最资深」。接口已在
      `Res.succeedCommander` 一处收口，要改只动那里
