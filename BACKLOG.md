...
- [ ] **L1** 1×1 置物货架建筑与仓储品类过滤 (#134)：`bl_storage_shelf` + `CFG.storage` 五大品类映射 + E 键单键预设轮转
- [ ] **L2** 掉落物露天劣化与天气腐烂模型 (#135)：100 HP 耐久模型 + 雨雪×2/酸雨×4加速腐烂 + 封闭房间与货架永久免疫
- [ ] **L3** 搬运工全图巡视与 48px 批量多堆抓取 (#136)：1200px 视野 + 48px 邻近多堆顺带拾取（至多3堆/50件）
- [ ] **L4** 车间工位 120px 就近取料与全场景冒烟闭环 (#137)：厨房/工坊/诊所/就餐 120px 就近货架优先取料 + 货架实物陈列渲染
